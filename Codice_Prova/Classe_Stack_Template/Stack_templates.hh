#ifndef EXCEPTION_SAFE_STACK_TEMPLATES_HH_INCLUDE_GUARD
#define EXCEPTION_SAFE_STACK_TEMPLATES_HH_INCLUDE_GUARD 1
...
Stack<T>::Stack(const Stack<U>& y)
  : vec_(new value_type[y.size() == 0 ? 1 : y.size()]),
    capacity_((y.size() == 0) ? 1 : y.size()),
    size_(y.size()) {
  for (size_type i = 0; i < y.size(); ++i) {
    // Per fare questo accesso a y.vec_ devo essere friend di Stack<U>.
    push(y.vec_[i]);
  }
}

#endif // EXCEPTION_SAFE_STACK_TEMPLATES_HH_INCLUDE_GUARD
