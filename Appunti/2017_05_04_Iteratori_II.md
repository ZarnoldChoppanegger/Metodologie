...
Algoritmo introspettivo, algoritmo che tiene traccia di quello che fa dentro se stesso. Es quicksort tiene traccia di quello che fa, se arriva a un tempo troppo lungo allora cambia l'argoritmo di ordinamento, per esempio se l'array diventa piccolo allora passa all'insertion sort.
...
* **_Compare** predicato che implementa il confronto, in questo modo non c'è più il bisogno di usare il <. La relazione di ordinamento non è banale: per esempio cosa ci dice che se confronto due stringe con < e > allora sono = ? Non è detto il criterio di confronto sia totale. Quindi devo stare attento al criterio di confronto che viene usato.
...
* se assumo che le sequenze abbiano la stessa sequenza non metto il secondo iteratore di fine
...
Tipi proxy si comportano come valuetype ma si comportano in maniera diversa.
...
* **iterator**, iteratore sugli elementi del vector, non mi interessa come è implementato. È un iteratore a value_type che consente di modificare gli elementi, corrisponde a concetto di puntatore sul quale però posso fare operatori(andare avanti e indietro):
...
* **difference_type**, corrisponde più o meno a size_type, ma con ha segno, serve a indicare distanza tra due elemnti di un vector.
...
``` `c++
	...
	//se avessi scritto
	auto first = vi.begin();
	//non c'è autodeduzione del tipo e quindi non capisce che deve
	//essere costante, quindi diventa lungo da scrivere
	
	stampa_tutti<int>::const_iterator last = vi.cend();
```
Quando la gente implementa i const_iterator mette anche un costruttore che fa conversione da iteratore non costante a costante. Allora, hanno dato due funzioni supplementari per consentire al programmatore di farsi dare l'iteratore costante. Così evito costo della copia e conversione.
...
```
Questo codice è pieno di UB, perchè non so quale è la capacità del vector, io faccio inserimenti se rialloca invalido TUTTI gli iteratori che sto usando (i, i_end).
